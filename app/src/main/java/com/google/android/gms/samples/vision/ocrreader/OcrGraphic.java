...
public class OcrGraphic extends GraphicOverlay.Graphic {
    ...
    private static final int RECT_COLOR = Color.WHITE;
    ...
    OcrGraphic(GraphicOverlay overlay, TextBlock text) {
        ...
        if (sRectPaint == null) {
            ...
            sRectPaint.setStrokeWidth(5.0f);
        }

        if (sTextPaint == null) {
            sTextPaint = new TextPaint();
            sTextPaint.setColor(TEXT_COLOR);
            sTextPaint.setTextSize(60.0f);
    ...
    public void draw(Canvas canvas) {
        ...
        for(Text currentText : textComponents) {
            Rect currentTextRect = currentText.getBoundingBox();
            float left = translateX(currentTextRect.left + (currentTextRect.width() / 2) * sTextPaint.getTextScaleX());
            float bottom = translateY(currentTextRect.bottom);
